...
class AIServerSettings(BaseSettings):
    ...
    # --- 商品向量索引（HuggingFace / sentence-transformers）---
    # 输出维度须与 core.ai.data.models.product_vector.ProductVector 中 Vector 维度一致（当前为 768）
    product_embedding_model_name: str = Field(
        default="sentence-transformers/all-mpnet-base-v2",
        description="商品同步到向量库时使用的句向量模型名称",
    )
...
class Settings(DatabaseSettings, AIServerSettings):
    ...
    # --- pydantic_settings配置，可配置 Pydantic 的运行时行为 ---
    # model_config 是父类 BaseSettings 的属性，类型为 SettingsConfigDict
